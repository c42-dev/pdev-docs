...
<AccordionGroup>
  ...
  <Accordion title="PIX QR Code Estático">
  ...
  <Accordion title="Withdrawals (Saques)">
    Criar saques via PIX. Informe **`amount`** (valor bruto debitado do saldo; destinatário recebe `amount - fee`) **ou** **`netAmount`** (valor líquido que o destinatário recebe; saldo é debitado em `netAmount + fee`) — nunca os dois.

    ```typescript
    // Por valor bruto (sai 150,75 do saldo; destinatário recebe 150,75 - fee)
    await pdev.withdrawals.create({ amount: 150.75, bankAccountId: 'uuid' });

    // Por valor líquido (destinatário recebe 150,00 exatos; saldo debita 150,00 + fee)
    await pdev.withdrawals.create({ netAmount: 150.0, bankAccountId: 'uuid' });
...
<CardGroup cols={3}>
  ...
  <Card title="SDK PHP" icon="php" href="https://github.com/MountBit/pague-dev-php-sdk">
