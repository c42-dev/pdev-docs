...
Uma cobrança PIX pode repartir parte do valor recebido com outras subcontas da mesma conta. Envie o campo opcional `split` na [criação da cobrança](/api-reference/pix/create):
...
| `split[].subAccount` | string | `reference` da subconta que recebe o rateio |
| `split[].amount` | number | Valor em reais destinado a essa subconta — mínimo `0.01`, no máximo 2 casas decimais |
...
### Só subcontas da mesma conta

`subAccount` só aceita o `reference` de uma subconta **ativa da mesma conta**. Split para fora da titularidade não existe nesta API.

A conta principal **não é endereçável como destino**: `reference` é uma propriedade de subconta, e a conta principal não tem um. Uma carteira que precisa devolver saldo para a principal usa a transferência interna — operação explícita, feita no Dashboard, que não fica pendurada na liquidação de um pagamento.
...
- uma **saída** no originador — `Split enviado para loja-centro`
