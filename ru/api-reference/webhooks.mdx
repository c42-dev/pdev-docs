...
| `payment_failed` | Платёж получен, но отклонён — депозит заблокирован политикой счёта (например, плательщик CNPJ) |
...
Отправляется, когда платёж PIX **получен, но не мог быть зачислен** — сейчас это случай, когда счёт блокирует депозиты от плательщиков-юрлиц (CNPJ). Сумма не поступает на ваш баланс, комиссия не взимается, деньги автоматически возвращаются плательщику. `blockedDeposit.refund` показывает ход возврата.
...
<Note>
Возврат выполняет платёжное учреждение сразу после блокировки. `refund.status` в вебхуке — состояние на момент отправки; E2E возврата (`refund.e2eId`) обычно приходит через несколько секунд — запросите `GET /transactions/{id}` для итоговых данных и ответа плательщику.
</Note>
...
| `transactionId` | string | Уникальный ID транзакции |
| `environment` | string | Окружение (`production` или `sandbox`) |
| `amount` | number | Сумма, уплаченная плательщиком |
| `feeAmount` | number | Взимаемая комиссия (всегда 0 — ничего не зачислено) |
| `netAmount` | number | Равно `amount` (справочно; ничего не зачислено) |
| `currency` | string | Валюта (BRL) |
| `paymentMethod` | string | Способ оплаты (pix) |
| `status` | string | Статус платежа (failed) |
| `failureReason` | string | Причина, текстом |
| `failedAt` | string | Дата и время отклонения в формате ISO 8601 |
| `createdAt` | string | Дата и время создания платежа в формате ISO 8601 |
| `e2eId` | string | Сквозной ID полученного PIX (необязательно) |
| `counterpartName` | string | Имя плательщика по данным PSP (необязательно) |
| `counterpartDocument` | string | CNPJ плательщика, без маски |
| `blockedDeposit.reason` | string | Причина блокировки: `cnpj_payer_not_allowed` |
| `blockedDeposit.refund.status` | string | Возврат плательщику: `pending` (принят, не подтверждён), `confirmed` (проведён) или `failed` (обрабатывается вручную) |
| `blockedDeposit.refund.e2eId` | string | Сквозной ID возврата (`D…`), если уже известен |
| `blockedDeposit.refund.settledAt` | string | Дата и время проведения возврата в формате ISO 8601, если подтверждён |
| `externalReference` | string | Ваш внешний референс (необязательно) |
| `customer` | object | Данные клиента, указанные при создании платежа (необязательно) |
| `metadata` | object | Пользовательские метаданные, отправленные при создании платежа (необязательно) |
