...
A PIX charge can split part of the amount it receives with other sub-accounts of the same account. Send the optional `split` field when [creating the charge](/en/api-reference/pix/create):
...
| `split[].subAccount` | string | `reference` of the sub-account that receives the share |
| `split[].amount` | number | Amount in BRL allocated to that sub-account — minimum `0.01`, at most 2 decimal places |
...
### Only sub-accounts of the same account

`subAccount` only accepts the `reference` of an **active sub-account of the same account**. Splitting outside the ownership does not exist in this API.

The main account is **not addressable as a destination**: `reference` is a property of a sub-account, and the main account does not have one. A wallet that needs to return balance to the main account uses an internal transfer — an explicit operation, done in the Dashboard, that is not attached to the settlement of a payment.
...
- an **outgoing** line on the originator — `Split enviado para loja-centro`
