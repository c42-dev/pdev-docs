...
| `payment_failed` | Payment received but refused — deposit blocked by an account policy (e.g. CNPJ payer) |
...
Sent when a PIX payment was **received but could not be credited** — today, when the account blocks deposits from business (CNPJ) payers. The amount does not enter your balance, no fee applies, and it is automatically returned to the payer. `blockedDeposit.refund` reports the return progress.
...
<Note>
The return is made by the payment institution right after the block. The webhook's `refund.status` is the state at send time; the return E2E (`refund.e2eId`) usually arrives seconds later — query `GET /transactions/{id}` for the final data and to answer the payer.
</Note>
...
| `transactionId` | string | Unique transaction ID |
| `environment` | string | Environment (`production` or `sandbox`) |
| `amount` | number | Amount paid by the payer |
| `feeAmount` | number | Fee charged (always 0 — nothing was credited) |
| `netAmount` | number | Same as `amount` (reference; nothing was credited) |
| `currency` | string | Currency (BRL) |
| `paymentMethod` | string | Payment method (pix) |
| `status` | string | Payment status (failed) |
| `failureReason` | string | Reason, as text |
| `failedAt` | string | Refusal date/time in ISO 8601 |
| `createdAt` | string | Charge creation date/time in ISO 8601 |
| `e2eId` | string | End-to-end ID of the received PIX (optional) |
| `counterpartName` | string | Payer name as returned by the PSP (optional) |
| `counterpartDocument` | string | Payer CNPJ, unmasked |
| `blockedDeposit.reason` | string | Block reason: `cnpj_payer_not_allowed` |
| `blockedDeposit.refund.status` | string | Return to the payer: `pending` (accepted, not yet confirmed), `confirmed` (settled) or `failed` (being handled manually) |
| `blockedDeposit.refund.e2eId` | string | End-to-end ID of the return (`D…`), when already known |
| `blockedDeposit.refund.settledAt` | string | Return settlement date/time in ISO 8601, when confirmed |
| `externalReference` | string | Your external reference (optional) |
| `customer` | object | Customer data provided when the charge was created (optional) |
| `metadata` | object | Custom metadata sent when the payment was created (optional) |
