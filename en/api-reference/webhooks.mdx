...
| `X-Misespay-Event` | Event type (e.g. `payment_completed`) |
| `X-Misespay-Webhook-ID` | Unique ID of this delivery (UUID) |
| `X-Misespay-Signature` | Payload signature: `sha256=<hex-encoded HMAC-SHA256>` |
| `X-Misespay-Timestamp` | Delivery timestamp in milliseconds (Unix epoch) |
...
3. Compare it with the `X-Misespay-Signature` header using a constant-time comparison
